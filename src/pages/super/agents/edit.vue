  ...
  <view class="agent-edit">
    <view class="card">
      ...
      <view class="form-item">
        ...
        <!-- 产品组合选择 -->
        <view v-if="productGroups.length > 0" class="product-group-select">
          <text class="form-desc" style="margin-bottom: 16rpx;">或直接选择已创建的产品组合</text>
          <view class="group-list">
            <view 
              v-for="group in productGroups"
              :key="group.id"
              class="group-item"
              :class="{ 'group-item--active': isGroupSelected(group.id) }"
            >
              <view class="group-item__left" @tap="selectProductGroup(group)">
                <view class="group-item__check">
                  <text v-if="isGroupSelected(group.id)">✓</text>
                </view>
                <view class="group-item__info">
                  <text class="group-item__name">{{ group.name }}</text>
                  <text class="group-item__products">
                    {{ group.productIds.map(id => getProductName(id)).join('、') }}
                  </text>
                </view>
              </view>
              <view v-if="isGroupSelected(group.id)" class="group-item__target" @tap.stop>
                <view class="quantity-control">
                  <view class="quantity-btn quantity-btn--small" @tap="changeGroupTargetQuantity(group.id, -1000)">-</view>
                  <input 
                    type="number" 
                    :value="getGroupTargetValue(group.id)" 
                    class="quantity-input quantity-input--small"
                    @input="(e: any) => setGroupTargetValue(group.id, Number(e.detail?.value ?? (e.target as HTMLInputElement)?.value ?? 0))"
                  />
                  <view class="quantity-btn quantity-btn--small" @tap="changeGroupTargetQuantity(group.id, 1000)">+</view>
                </view>
                <text class="group-item__unit">箱</text>
              </view>
            </view>
          </view>
        </view>
        ...
        <view v-if="selectedProducts.length > 1" class="group-target-section">
          <text class="group-target-label">手动组合目标（共{{ selectedProducts.length }}个产品）</text>
...
const handleCropConfirm = async (croppedImagePath: string) => {
  ...
  try {
    // 上传裁剪后的图片到服务器
    const imageUrl = await uploadImage(croppedImagePath)

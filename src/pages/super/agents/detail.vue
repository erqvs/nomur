  ...
  <view class="agent-detail" v-if="agent">
    ...
    <view v-if="showGiftsModal" class="modal-mask" @tap="showGiftsModal = false">
      <view class="modal-content modal-content--large" @tap.stop>
            ...
            <view class="gifts-detail-list">
              ...
              >
                ...
                <view class="gift-detail-item__control">
                  <text class="gift-detail-item__label">已送达：</text>
                  <view class="quantity-control">
                    <view class="quantity-btn quantity-btn--small" @tap="changeGiftDeliveredQuantity(gift, -1)">-</view>
                    <input 
                      type="number" 
                      :value="gift.deliveredQuantity" 
                      class="quantity-input quantity-input--small"
                      @input="(e: any) => setGiftDeliveredQuantity(gift, Number(e.detail?.value ?? (e.target as HTMLInputElement)?.value ?? 0))"
                    />
                    <view class="quantity-btn quantity-btn--small" @tap="changeGiftDeliveredQuantity(gift, 1)">+</view>
                  </view>
        ...
        <view class="modal-actions">
          <view class="modal-btn modal-btn--cancel" @tap="showGiftsModal = false">关闭</view>
          <view class="modal-btn modal-btn--confirm" @tap="saveGifts">保存</view>
        </view>
      </view>
    </view>
...
const recentRecords = computed(() => {
  ...
  const transactions = store.getAgentTransactions(agentId.value).map(tx => {
    ...
    if (tx.reason === 'shipping' && tx.relatedOrderId) {
      ...
      if (relatedOrder && relatedOrder.giftItems && relatedOrder.giftItems.length > 0) {
...
const goToEdit = () => {
  uni.navigateTo({
    url: `/pages/super/agents/edit?id=${agentId.value}`
  })
}

const goToOrders = () => {
  uni.navigateTo({
    url: `/pages/super/orders/list?agentId=${agentId.value}`
  })
}
...
const changeGiftDeliveredQuantity = (gift: GiftStatus, delta: number) => {
  const key = gift.isGroup ? `group:${gift.groupId}` : `product:${gift.productId}`
  const current = giftsForm.value.get(key) ?? gift.deliveredQuantity
  
  const newValue = Math.max(0, Math.min(gift.totalQuantity, current + delta))
  giftsForm.value.set(key, newValue)
  
  // 更新显示
  gift.deliveredQuantity = newValue
  gift.undeliveredQuantity = gift.totalQuantity - newValue
}
...
const setGiftDeliveredQuantity = (gift: GiftStatus, value: number) => {
  ...
  const newValue = Math.max(0, Math.min(gift.totalQuantity, value || 0))
  giftsForm.value.set(key, newValue)
...
const goToEditTransaction = () => {
  ...
  uni.navigateTo({
    url: `/pages/super/transactions/edit?id=${selectedRecord.value.id}`
  })
}

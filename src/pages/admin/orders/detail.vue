      ...
      <view v-if="order.giftItems && order.giftItems.length > 0 && deliveryRecords.length > 0" class="card">
        ...
        <view class="delivery-records-list">
          ...
          >
            ...
            <view class="delivery-record-actions">
              <view class="delete-btn" @tap.stop="deleteDeliveryRecord(record.id)">
                <image src="/static/icons/trash.svg" class="delete-icon" mode="aspectFit" />
...
// 删除送达记录
const deleteDeliveryRecord = async (recordId: string) => {
  try {
    // 检查管理员信息
    ...
    const result = await uni.showModal({
      title: '确认删除',
      content: '确定要删除这条送达记录吗？删除后，订单中的已送达数量会相应减少。',
      confirmText: '删除',
      confirmColor: '#FF4D4F',
      cancelText: '取消'
    })
    ...
    await orderApi.deleteGiftDeliveryRecord(
      orderId.value, 
      recordId,
...
const saveSingleGift = async (gift: GiftStatus) => {
  ...
  const pendingQuantity = getPendingDeliveredQuantity(gift) // 本次要送达的数量
  // 使用原始值作为基准
  const original = originalGifts.value.get(gift.productId)
  if (!original) {
...
.delete-icon {
  width: 28rpx;
  height: 28rpx;
  filter: brightness(0) saturate(100%) invert(30%) sepia(95%) saturate(7471%) hue-rotate(348deg) brightness(99%) contrast(101%);
}
